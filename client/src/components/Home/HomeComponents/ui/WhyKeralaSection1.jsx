...
const WhyKeralaSection1 = () => {
  ...
  return (
    <div className="py-25 px-5 bg-[linear-gradient(150deg,#244C38_20%,#203c2e_80%)]">
      {/* section 1 */}
      <div className="text-center mb-14  text-[#FCFAF8]  ">
        <h3 className="text-sm sm:text-base inter uppercase tracking-[0.25em] text-[#4DBD89] mb-2">
          The Perfect Destination
        </h3>
        <h1 className="text-3xl sm:text-5xl playfair font-semibold  mb-4">
          Why Kerala for Healthcare & Ayurveda
        </h1>
        <p className="inter w-[50%] mx-auto text-[#FCFAF8]  text-[16px] font-light ">
          Kerala offers a unique blend of nature, traditional medicine, and
          modern healthcare. <br /> Its climate, wellness culture, and medical
          expertise create an ideal <br /> environment for healing and recovery.
        </p>
      </div>
      {/* section 2 */}
      <div className="flex justify-center items-center gap-10 text-[#FCFAF8] text-[13px] inter ">
        {reasons.map((reason) => (
          <div key={reason.id} className="flex flex-col items-center gap-2">
            <img src={reason.image} alt={reason.title} />
            <p className="text-center">{reason.title}</p>
          </div>
        ))}
      </div>
    </div>
